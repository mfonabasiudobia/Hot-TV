...
namespace App\Http\Livewire\Admin\RideDuration\Tables;
    ...
    */
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ->addColumn('id')
            ->addColumn('duration')
            ->addColumn('price_formatted', function(RideDuration $model) {
                return '$'.$model->price;
            })
            ->addColumn('stream_formatted', function(RideDuration $model){
                $stream = $model->stream ? 'Yes' : 'No';
                return "<span class='px-3 py-1 rounded-full bg-green-800'>". $stream ."</span>";
            })
            ->addColumn('status_formatted', function(RideDuration $model){
    ...
    */
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Stream', 'stream_formatted'),
            Column::make('Created At', 'created_at_formatted'),
        ];
    }

    public function onUpdatedEditable($id, $field, $value): void{
        RideDuration::query()->find($id)->update([
            $field => $value,
        ]);
    }
...
//                 ->route('admin.podcast.show', ['slug' => 'slug']),
            ...
            Button::add('edit')
                ...
                ->class('bg-gray-600 cursor-pointer text-white px-3 py-1 m-1 rounded text-sm')
                ->target('_self')
                ->route('admin.ride.edit-duration', ['id' => 'id']),
