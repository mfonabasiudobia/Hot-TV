<div class="py-5 bg-black text-white space-y-5 min-h-screen">
    <x-atoms.breadcrumb :routes="[['title' => 'Search', 'route' => null ]]" />
    <div class="container space-y-7">

        <section class="space-y-7">
            <header class="space-y-3">
                <h2 class="font-semibold text-xl md:text-3xl">Search</h2>
                <p>Results for search term: {{ $q }}</p>
            </header>
            ...
            <div class="rounded-2xl px-7 hidden md:flex justify-between items-center bg-dark text-sm py-3">
                ...
                <div class="flex items-center space-x-7">
                    ...
                    <button wire:click="sortByDate()" class="flex items-center space-x-1">
                        <i class="las {{ $sortByDate === " desc" ? 'la-sort-amount-down-alt' : 'la-sort-amount-up-alt'
                            }} text-xl"></i>
            ...
            <section class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
                ...
                <a class="relative space-y-5 p-5 bg-dark rounded-xl group hover:scale-125 transition-all"
                    href="{{ route('tv-shows.show', ['slug' => $show->slug ]) }}">
                    <img src="{{ file_path($show->thumbnail) }}" alt=""
                    ...
                    <span
                        ...
                        {{ $show->categories()->where('name', 'like', '%' . $q . '%')->first()->name ?? $show->categories[0]->name }}
                    </span>
            ...
            <div class="flex justify-center md:justify-end text-sm">
                ...
                <div class="flex flex-col md:flex-row items-center space-y-3 md:space-y-0 md:space-x-7">
                    <span class="opacity-60">Showing {{ $tvShows->firstItem() }} to {{ $tvShows->lastItem() }} of {{
                        $tvShows->total()
                        }} data</span>

                    <div class="space-x-1 flex items-center">
                        @if ($tvShows->previousPageUrl())
                        <button wire:click="gotoPage({{ $tvShows->currentPage() - 1 }})"
