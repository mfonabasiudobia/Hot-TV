<div class="modal-wrapper text-white" x-data="{ status : false }" @trigger-change-avatar-modal.window="status = !status;"
            ...
            <form wire:submit.prevent='submit' class="grid gap-5" autocomplete="off">
                ...
                <div class="form-group">
                    ...
                    <x-atoms.progress-indicator>
                        <input type="file" class="form-control" accept="image" wire:model.defer="avatar_file" />
                    </x-atoms.progress-indicator>
                    @error('avatar_file') <span class="error"> {{ $message }}</span> @endError
                </div>
