<section>
    ...
    <section class="space-y-5">
        <h2 class="font-medium text-xl capitalize">Add New {{ $tvshow->title }} Cast</h2>

        <section>
            <form class="grid md:grid-cols-2 gap-5" wire:submit.prevent='submit'>
                <div class="form-group md:col-span-2">
                    <label>Name</label>
                    <input type="text" placeholder="Name" wire:model.defer='name' class="form-control-light" />
                    @error('name') <span class="error">{{ $message }}</span> @endError
                </div>

                <div class="form-group md:col-span-2">
                    <label>Character Role</label>
                    <input type="text" placeholder="Character Role" wire:model.defer='role' class="form-control-light" />
