    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table->eloquent($this->query())
            ...
            ->editColumn('name', function (Service $service) {
                if (! $this->hasPermission('business-services.services.edit')) {
                    return BaseHelper::clean($service->name);
                }

                return Html::link(route('business-services.services.edit', $service->getKey()), BaseHelper::clean($service->name));
            });
