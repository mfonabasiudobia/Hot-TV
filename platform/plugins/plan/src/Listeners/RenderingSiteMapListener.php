...
namespace Botble\Plan\Listeners;

use Botble\Base\Enums\BaseStatusEnum;
use Botble\Plan\Repositories\Interfaces\PostInterface;
...
class RenderingSiteMapListener
{
    ...
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            ...
            if (preg_match('/^Plan-posts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
        ...
        foreach ($posts as $post) {
            $key = sprintf('Plan-posts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        SiteMapManager::addSitemap(SiteMapManager::route('Plan-categories'), $categoryLastUpdated);
        ...
        SiteMapManager::addSitemap(SiteMapManager::route('Plan-tags'), $tagLastUpdated);
    }
}
