...
namespace Botble\Stream\Listeners;
...
class RenderingSiteMapListener
{
    ...
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            ...
            if (preg_match('/^Stream-posts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
                if (($year = Arr::get($matches, 1)) && ($month = Arr::get($matches, 2))) {
                    $posts = $this->streamRepository->getModel()
        ...
        foreach ($posts as $post) {
            $key = sprintf('Stream-posts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        SiteMapManager::addSitemap(SiteMapManager::route('Stream-categories'), $categoryLastUpdated);
        ...
        SiteMapManager::addSitemap(SiteMapManager::route('Stream-tags'), $tagLastUpdated);
    }
}
