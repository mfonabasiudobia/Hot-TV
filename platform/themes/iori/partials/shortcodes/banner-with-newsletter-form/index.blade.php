            ...
            <div class="row align-items-center">
                <div class="col-lg-7">
                    @if ($subtitle = $shortcode->subtitle)
                        <span class="title-line line-48 wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $subtitle }}</span>
                    @endif

                    @if ($title = $shortcode->title)
                        <h1 class="color-brand-1 mb-20 mt-10 wow animate__animated animate__fadeIn" data-wow-delay=".2s">{{ $title }}</h1>
                    @endif
                    <div class="row wow animate__animated animate__fadeIn" data-wow-delay=".4s">

                    @if ($description = $shortcode->description)
                        <div class="col-lg-10">
                            <p class="font-md color-grey-500">{{ $description }}</p>
                        </div>
                    @endif
