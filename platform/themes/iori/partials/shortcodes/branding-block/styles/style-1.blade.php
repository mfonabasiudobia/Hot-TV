    ...
    <div class="row align-items-center">
        ...
        <div class="col-lg-5">
            @if ($subtitle = $shortcode->subtitle)
                <span class="btn btn-tag wow animate__animated animate__fadeInUp" data-wow-delay=".0s">{{ $subtitle }}</span>
            @endif

            @if ($title = $shortcode->title)
                <h3 class="color-brand-1 mt-10 mb-15 wow animate__animated animate__fadeInUp">{{ $title }}</h3>
            @endif

            @if ($description = $shortcode->description)
                <p class="font-md color-grey-400 wow animate__animated animate__fadeInUp">{{ $description }}</p>
            @endif
