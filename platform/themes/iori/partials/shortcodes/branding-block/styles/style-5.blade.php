            ...
            <div class="row align-items-center">
                <div class="col-lg-5">
                    @if ($subtitle = $shortcode->subttile)
                        <span class="btn btn-tag wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $subtitle }}</span>
                    @endif

                    @if ($title = $shortcode->title)
                        <h3 class="color-brand-1 mt-10 mb-15 wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $title }}</h3>
                    @endif

                    @if ($description = $shortcode->description)
                        <p class="font-md color-grey-400 wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $description }}</p>
                    @endif
                    <div class="mt-20">
                        <ul class="list-ticks">
                            @foreach($tabs as $tab)
