        ...
        <div class="container">
            <div class="text-center">
                @if ($title = $shortcode->title)
                    <h3 class="color-brand-1 mb-15 wow animate__animated animate__fadeInUp" data-wow-delay=".0s">{{ $title }}</h3>
                @endif

                @if ($subtitle = $shortcode->subtitle)
                    <p class="font-lg color-grey-500 wow animate__animated animate__fadeInUp" data-wow-delay=".0s">{{ $subtitle }}</p>
                @endif
            </div>
