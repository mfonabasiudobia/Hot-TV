    ...
    <div class="container">
        <div class="text-center">
            @if ($title = $shortcode->title)
                <h2 class="color-brand-1 mb-40 wow animate__animated animate__fadeInUp" data-wow-delay=".0s">{{ $title }}</h2>
            @endif
        </div>
        <div class="table-responsive table-responsive-sm table-responsive-md">
            <table class="table table-compare">
                <thead>
                <tr>
                    <th class="width-28 color-success">{{ __('Data') }}</th>
                    <th class="width-18">{{ __('Free') }}</th>
                    <th class="width-18">{{ __('Standard') }}</th>
                    <th class="width-18">{{ __('Business') }}</th>
                    <th class="width-18">{{ __('Enterprise') }}</th>
                </tr>
                </thead>
                <tbody>
                    @foreach($tabs as $tab)
