                ...
                <div class="col-xl-6">
                    <div class="box-banner-left-home7">
                        @if ($subtitle = $shortcode->subtitle)
                            <span class="title-line line-48 wow animate__animated animate__fadeInUp" data-wow-delay=".s">{{ $subtitle }}</span>
                        @endif

                        @if ($title = $shortcode->title)
                            <h1 class="color-brand-1 mb-20 mt-5 wow animate__animated animate__fadeInUp" data-wow-delay=".1s">{{ $title }}</h1>
                        @endif
                        <div class="row">
                        @if ($description = $shortcode->description)
                            <div class="col-lg-10">
                                <p class="font-md color-grey-500 mb-25 wow animate__animated animate__fadeInUp" data-wow-delay=".2s">{{ $description }}</p>
                            </div>
                        @endif
                        </div>
