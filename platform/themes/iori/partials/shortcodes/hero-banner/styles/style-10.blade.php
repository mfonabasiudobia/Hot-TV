        ...
        <div class="row mt-150">
            <div class="col-xl-8 col-lg-10 m-auto">
                @if ($subtitle = $shortcode->subtitle)
                    <span class="font-md color-grey-400 wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $subtitle }}</span>
                @endif

                @if ($title = $shortcode->title)
                    <h1 class="color-brand-1 mb-25 mt-10 wow animate__animated animate__fadeIn" data-wow-delay=".2s">{{ $title }}</h1>
                @endif

                @if ($description = $shortcode->description)
                    <p class="font-md color-grey-500 mb-25 wow animate__animated animate__fadeIn" data-wow-delay=".4s">{{ $description }}</p>
                @endif
