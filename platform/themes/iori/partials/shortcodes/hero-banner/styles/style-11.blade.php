                ...
                <div class="col-xxl-6 col-xl-7 col-lg-12">
                    <div class="box-banner-service">
                        @if($title = $shortcode->title)
                            <h1 class="color-brand-1 mb-2 wow animate__animated animate__fadeIn" data-wow-delay=".0s">{{ $title }}</h1>
                        @endif
                        <div class="row">
                            @if ($description = $shortcode->description)
                                <div class="col-lg-9">
                                    <p class="font-md color-grey-500 wow animate__animated animate__fadeIn" data-wow-delay=".2s">{{ $description }}</p>
                                </div>
                            @endif
                        </div>
                        <div class="mt-30 wow animate__animated animate__fadeIn" data-wow-delay=".4s">
                            @if ($subtitle = $shortcode->subtitle)
                                <h5 class="color-brand-1">{{ $subtitle }}</h5>
                            @endif
                        </div>
