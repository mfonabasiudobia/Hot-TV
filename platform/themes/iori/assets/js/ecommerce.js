class Ecommerce {
    ...
    init() {
        this.$body
            ...
            .on('click', '.filter-layout', (event) => {
                ...
                this.$productsFilter.find('input[name=layout').val($(currentTarget).closest('.filter-link').data('layout'))
                this.$productsFilter.trigger('submit')
    ...
    reviewSection() {
        $('.anchor-link').on('click', function(e) {
            ...
            $(`${target}-pane`).addClass('active show').siblings().removeClass('active show');
        ...
        if (window.location.hash === '#review-tab') {
            $('.anchor-link').trigger('click');
        }

        let $reviewListWrapper = this.$body.find('.comment-list');
        ...
        initLightgallery($('.product-review-images'))
        ...
        $reviewListWrapper.on('click', '.pagination .page-item a', function (e) {
            e.preventDefault()
    ...
    handleProductsSorting(event) {
        ...
        this.$productsFilter.find('input[name="sort-by"]')
    ...
    handleProductsPerPage(event) {
        ...
        this.$productsFilter.find('input[name="per-page"]')
            .val($currentTarget.val())
            .change()
    }
    ...
    handleProductsPagination(event) {
        ...
        this.$productsFilter.find('input[name="page"]')
            .val(page)
            .change()
    }
    ...
    filterProducts(event) {
        ...
        $.ajax({
            ...
            success: (response) => {
                ...
                this.$body.find('.show-information-product').html(this.$body.find('.product-list').find('.showing-product').html())
